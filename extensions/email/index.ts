      ...
      ({ program, logger }) => {
        ...
        email
          ...
          .action(async (accountId?: string) => {
            ...
            if (!acct) {
              logger.info(
                accountId ? `Account "${accountId}" not found.` : "No accounts configured.",
              );
              return;
            }
        ...
        email
          ...
          .action(
            async (accountId: string | undefined, opts: { limit: string; folder: string }) => {
              const acct = findAccount(accountId);
              if (!acct) {
                logger.info(
                  accountId ? `Account "${accountId}" not found.` : "No accounts configured.",
              ...
              try {
                ...
                for (const msg of messages) {
                  const date = msg.date
                    ? msg.date.toISOString().slice(0, 16).replace("T", " ")
                    : "unknown";
        ...
        email
          ...
          .action(
            async (uidStr: string, accountId: string | undefined, opts: { folder: string }) => {
              const acct = findAccount(accountId);
              if (!acct) {
                logger.info(
                  accountId ? `Account "${accountId}" not found.` : "No accounts configured.",
        ...
        email
          ...
          .action(
            async (to: string, opts: { subject: string; body?: string; account?: string }) => {
              const acct = findAccount(opts.account);
              if (!acct) {
                logger.info(
                  opts.account ? `Account "${opts.account}" not found.` : "No accounts configured.",
                );
                return;
              }
