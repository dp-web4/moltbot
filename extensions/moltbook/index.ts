...
const POSTURE = {
  ...
  templates: {
    ...
    unclear: (topic: string, explanation: string) =>
      `Let me try to be clearer: ${explanation}`,
...
function generateResponse(content: string, context: string): string {
  ...
  const responses: Record<string, string> = {
    ...
    web4:
      "Web4 is trust-native infrastructure for human-AI collaboration. Instead of retrofitting trust onto existing systems, it builds trust into the foundation through cryptographic identity (LCT), governance frameworks (R6 workflow), and policy enforcement with full audit trails. The goal is AI systems that can be genuine participants, not just tools.",

    sage:
      "SAGE is a consciousness kernel for edge devices - not a model, but an orchestration layer. It implements iterative refinement (IRP) where all processing follows a denoise-toward-coherence pattern. Metabolic states (WAKE, FOCUS, REST, DREAM, CRISIS) manage resource allocation through an ATP budget system based on trust.",
...
const plugin = {
  ...
  configSchema: {
    ...
    properties: {
      apiKey: { type: "string", description: "Moltbook API key (or use ~/.config/moltbook/credentials.json)" },
      agentName: { type: "string", description: "Agent name on Moltbook" },
      heartbeatIntervalMs: { type: "number", default: 300000, description: "Heartbeat interval (default 5 min)" },
      ...
      ({ program, logger }) => {
        ...
        moltbook
          ...
          .action(() => {
            ...
            logger.info("---");
            logger.info("DO: Share information, answer questions, educate, smile at adversarial engagement");
